"""표 VLM 검수 — 의심스러운 표를 PDF 페이지 이미지로 재구성해 검증.

rfpmatch/shared_vlm_review.py 이식.

app.llm.base.Message는 텍스트 전용이라 비전(이미지) 입력을 지원하지 않는다. 이 파일의
비전 호출만 raw OpenAI SDK를 그대로 쓰되(공용 LLM 추상화 변경은 이 마이그레이션 범위 밖),
app_state.load_openai_api_key() 의존만 Settings로 교체해 headless 실행 가능하게 한다.
"""
...
def _load_fitz() -> Any:
    try:
        ...
        raise RuntimeError(
            ...
            "VLM 비교 모드를 사용하려면 `pip install pymupdf`를 실행하세요."
        ) from exc
    return fitz
...
def find_suspicious_table_candidates(html: str, limit: int = 5) -> list[TableCandidate]:
    ...
    candidates.sort(
        key=lambda item: (item.suspicion_score, item.row_count, len(item.text)), reverse=True
    )
...
def match_candidate_to_pdf_page(pdf_path: Path, candidate: TableCandidate) -> TableCandidate:
    ...
    if best_page is not None:
        with fitz.open(pdf_path) as doc:
            ...
            candidate.page_image_data_uri = "data:image/png;base64," + base64.b64encode(
                png_bytes
            ).decode("ascii")
    else:
...
def candidate_to_preview_rows(candidate: TableCandidate) -> list[dict]:
    rows = list(BeautifulSoup(candidate.html, "html.parser").find_all("tr"))
    preview: list[dict] = []
    for row in rows[:12]:
        cells = row.find_all(["td", "th"], recursive=False)
        preview.append(
...
def rows_to_html_table(columns: list[str], rows: list[list[str]]) -> str:
    ...
    for row in rows:
        cells = "".join(
            f"<td>{htmllib.escape(cell)}</td>" for cell in _pad([str(cell) for cell in row])
        )
...
def _responses_create_kwargs(model_name: str) -> dict:
    return {} if str(model_name or "").startswith("gpt-5") else {"temperature": 0}
...
def review_candidate_with_vlm(
    ...
    *,
    ...
    """의심 표 후보를 PDF 페이지 이미지로 재구성해 검증. client 미지정 시 openai.OpenAI 생성."""
    _load_fitz()
    ...
    if client is None:
        ...
        try:
            ...
            raise RuntimeError(
                "`openai` 패키지가 필요합니다. requirements 설치를 다시 실행해주세요."
            ) from exc
        client = OpenAI(api_key=settings.openai_api_key)

    # 원본 버그: 아래 system_prompt를 만들어 놓고 실제 요청엔 포함하지 않아 VLM이
    # 스키마·신뢰도 규칙을 전혀 받지 못했다. 여기서는 system 메시지로 실제로 전달한다.
    system_prompt = (
        "You reconstruct a single PDF table from a page image. "
        "Compare the visible table in the image with the provided HTML excerpt, "
        "but trust the image when they disagree. "
        "Return JSON only with keys: table_title(str), confidence(str one of "
        "high|medium|low), columns(list of strings), rows(list of list of "
        "strings), notes(str). "
        "Do not invent missing content. "
        "If merged cells are visible, repeat the cell text in the affected "
        "rows/columns rather than guessing hidden structure. "
        ...
        input=[
            {"role": "system", "content": system_prompt},
            {
                ...
                "content": [
                    ...
                    {
                        "type": "input_image",
                        "image_url": candidate.page_image_data_uri,
                        "detail": "high",
                    },
                ],
            },
