...
from rfpmatch.shared_models import TocItem
...
from rfpmatch.toc_parser import _detect_txt_toc_style
...
def estimate_llm_cost_usd(model_name: str, input_tokens: int, output_tokens: int) -> float:
    pricing = LLM_MODEL_PRICING_PER_1M.get(model_name, LLM_MODEL_PRICING_PER_1M["gpt-5.2"])
    return (input_tokens / 1_000_000) * pricing["input"] + (output_tokens / 1_000_000) * pricing["output"]


def responses_create_kwargs(model_name: str) -> dict:
    kwargs: dict = {}
...
def parse_llm_toc_json_rows(content: str) -> list[dict]:
...
def _strip_leading_toc_region_from_txt(raw_text: str) -> str:
    ...
    def _looks_like_body_heading(line: str) -> bool:
        ...
        if not compact:
            return False
        return bool(
            re.match(r"^(?:제?\s*\d+\s*(?:장|절|항)|\d+(?:\.\d+)+|[IVXLCDM]+|[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ]+)[\.\)]?\s+.+", compact, re.IGNORECASE)
...
def _infer_missing_pages_from_body(items: list[TocItem], raw_text: str) -> list[TocItem]:
    ...
    def _normalize_for_match(value: str) -> str:
        ...
        compact = re.sub(r"[.\-_·•\s]{2,}\d{1,4}\s*$", "", compact)
        return compact
    ...
    def _candidate_title_keys(title: str) -> list[str]:
        ...
        numbered_stripped = re.sub(r"^(?:제?\d+(?:\.\d+)*|[IVXLCDM]+|[가나다라마바사아자차카타파하]|[A-Za-z])[\.\)]?", "", stripped, flags=re.IGNORECASE)
...
def _drop_unsequenced_bullet_items(items: list[TocItem]) -> list[TocItem]:
    ...
    for item in items:
        ...
        if re.match(r"^[\-\*•·▪■◆▶◦○□◇※]", compact):
            ...
            if not bool(
                re.match(r"^(?:제?\s*\d+\s*(?:장|절|항)|\d+(?:\.\d+)*|[IVXLCDM]+)[\.\)]?\s+", stripped, re.IGNORECASE)
    ...
    for item in items:
        ...
        for candidate in prefix_candidates.get(current_prefix, []):
            ...
            if candidate_score <= best_score:
                continue
            if current_rest and not (current_rest in candidate_rest or candidate_rest in current_rest):
                continue
...
def _toc_level_guidance_prompt() -> str:
    return (
        ...
        "Do not flatten levels. Infer level from the heading numbering pattern, not from visual spacing alone. "
        "Use these rules as the default mapping unless the document clearly shows a different hierarchy:\n"
        "- Ⅰ, Ⅱ, Ⅲ, I., II., III., 제1장, 제1절, 제1항 and similar top section markers => level 1.\n"
        "- 1., 2., 3., 가., 나., 다., A., B. and similar first child markers under a section => level 2.\n"
        ...
        "- If the document contains a visible parent-child chain, keep the chain in the output hierarchy exactly as seen.\n"
        "- A more specific numbering pattern must never be assigned the same level as its parent.\n"
        "- If two headings differ only by numbering depth, the deeper one must have a strictly larger level.\n"
        "- Do not convert all numbered headings into the same level just because they appear in the same page or section.\n"
        "Examples:\n"
        "  I. 제안 요청 개요 -> level 1\n"
        "  1. 사업 개요 -> level 2\n"
        "  1.1. 프로젝트 개요 -> level 3\n"
        "  1.1.1. 세부 과업 -> level 4\n"
        "Keep the page key for every item. Preserve order. Remove duplicates only when the title and hierarchy are truly identical.\n"
    )


def build_llm_toc_from_raw_document(raw_text: str, source_name: str, model_name: str) -> tuple[list[TocItem], dict]:
    ...
    if not raw_text.strip():
        return [], {"model": model_name, "input_tokens": 0, "output_tokens": 0, "cost_usd": 0.0, "source": source_name}

    raw_text_original = raw_text
    txt_style = "document"
    if "txt" in source_name.lower():
        raw_text = _preprocess_txt_for_toc_llm(raw_text)
        txt_style = _detect_txt_toc_style(raw_text_original)
    ...
    system_prompt = (
        ...
        "Return only JSON array. Each item must have keys: level(int 1-6), title(str), page(int|null), anchor(str). "
        "The page field is mandatory for every item and must never be omitted from the JSON object. "
        "If the document contains an explicit table of contents section, prefer the page numbers shown in that TOC section for TOC items. "
        "If the document does not contain an explicit table of contents section, reconstruct the structure from the headings in the body text and infer each page from the first page marker that appears after the matching heading. "
        "If a page number is available in the document text or can be inferred from the nearby context, include it; otherwise use null but still include the page key. "
        "When inferring from body text, match the heading first and then take the first page marker that appears after that heading in the document flow. "
        "Infer the table of contents from the entire document text, preserve hierarchy, and remove duplicates. "
        "Use only the supplied document text. Do not add explanations. "
        "If a title in the document has leading numbering, the title you return must include that numbering exactly in the title field. "
        "If the document contains a separate table of contents section, do not use the TOC's own page numbers as evidence for final page fields; prefer page markers that appear in the body text after the heading. "
        "If a section contains dense sub-hierarchies or many bullets (such as 가, 나, 다, 1), 2), -, etc.), go one level deeper than usual to capture more granular sub-sections as TOC items, adding them with appropriate hierarchical levels (level 1-6) so no detailed content sections are missed. "
    ...
    if "txt" in source_name.lower() and txt_style == "ppt":
        system_prompt += (
            ...
            "Short standalone slide titles are meaningful TOC entries even when page numbers are missing. "
            "Prefer title-like fragments over long prose sentences."
        )
    txt_style_prompt = (
        "This TXT source looks like a presentation export. "
        "Treat short slide titles as valid TOC entries and do not require page numbers for them.\n\n"
        if "txt" in source_name.lower() and txt_style == "ppt"
        else ""
    )
    base_user_prompt = (
        ...
        "4) Keep hierarchy levels as accurately as possible and never flatten a deeper heading into the same level as its parent.\n"
        "5) Never omit the page key for any item.\n"
        "6) If a heading/title has numbering in the document, include that numbering in the returned title.\n"
        "7) If the document contains a TOC section, use the page numbers written in that TOC section for TOC items.\n"
        "8) If the document does not contain a TOC section, locate each heading in the body text and use the first page marker that appears after the matching heading.\n"
        "9) Do not use page numbers that belong to the TOC section itself as evidence for body-derived pages when the TOC and body disagree.\n"
        "10) If a section has multiple nested items or bullets, extract one more level of sub-bullets than usual to ensure detailed sub-items are captured.\n"
        "11) If a title starts with a bullet marker in the source, preserve that bullet-aware structure in the title/anchor so downstream card splitting can distinguish it.\n"
        "12) Follow the level guidance exactly.\n"
        "13) Return JSON array only.\n\n"
        f"{txt_style_prompt}"
        f"[RAW_DOCUMENT]\n{raw_text}"
    )
    ...
    while attempt_idx < len(prompt_attempts):
        ...
        final_rows = parse_llm_toc_json_rows(getattr(response, "output_text", "") or "")
        ...
        if not is_txt_source or not missing_sequences or attempt_idx >= 1:
            break
        numbered_titles = [item.title for item in final_items if extract_leading_numbering(item.title)]
        retry_prompt = (
            f"Rebuild the table of contents from the following {source_name} document text.\n"
            "Your previous TOC likely missed some numbering or omitted numbering prefixes in titles.\n"
            "Rules:\n"
            "1) Every title that has numbering in the document must include the numbering in the returned title.\n"
            "2) Review the numbering hierarchy in order and fill missing numbering items by searching the raw text again.\n"
            "3) Keep document order and preserve hierarchy.\n"
            "4) Never flatten a deeper heading into the same level as its parent.\n"
            "5) Never omit the page key for any item.\n"
            "6) If a section has multiple nested items or bullets, extract one more level of sub-bullets than usual to ensure detailed sub-items are captured.\n"
            "7) Follow the level guidance exactly.\n"
            "8) Return JSON array only.\n"
            f"Detected missing numbering sequences: {', '.join(missing_sequences)}.\n"
            f"Current numbered titles sample: {json.dumps(numbered_titles[:40], ensure_ascii=False)}\n\n"
            f"[RAW_DOCUMENT]\n{raw_text}"
        )
    ...
    return dedup_toc_items_by_title(completed_items), usage_info
