...
def _prepend_empty_column(table: Tag, count: int = 1) -> None:
    ...
    for tr in table.find_all("tr"):
        ...
        for _ in range(count):
            ...
            new_cell = BeautifulSoup(f"<{new_cell_name}></{new_cell_name}>", "html.parser").find(new_cell_name)
            if new_cell is None:
                continue
            new_cell.string = ""
...
def _is_effectively_empty_cell(cell: Tag | None) -> bool:
    ...
    if text:
        return False
    if cell.find(["img", "svg", "canvas"], recursive=True) is not None:
        return False
    return True
...
def _merge_table_fragments_preserving_outer_html(previous_html: str, current_html: str) -> str:
    ...
    merged_html = _merge_consecutive_table_html(str(prev_table), str(curr_table))
    return merged_html
    ...
    if prev_rows and len(prev_rows) == 1:
        ...
        prev_header_only = bool(
            ...
            and (
                ...
                or all(text and len(re.sub(r"\s+", "", text)) <= 20 and not re.search(r"[.?!]$", text) for text in first_row_texts)
            )
        )
...
def _is_transparent_table_gap(node: Tag) -> bool:
    ...
    if lowered.startswith("table") and len(lowered.split()) <= 4:
        return True
    return False
...
def _should_merge_tables(previous_table: Tag, current_table: Tag) -> bool:
    ...
    if _table_first_row_leading_blank_count(current_table) > 0:
        return True
...
def _strip_table_labels_from_section(node: Tag) -> None:
    ...
    if meta is not None:
        ...
        if text.startswith("table") or text.startswith("page:"):
    ...
    while True:
        ...
        def _nearest_non_empty_above(r_idx: int, c_idx: int) -> Tag | None:
            for probe_row in range(r_idx - 1, -1, -1):
                ...
                if candidate is None or candidate is grid.get((r_idx, c_idx)):
                    continue
...
def _merge_empty_cells_downward(table: Tag) -> bool:
    ...
    while True:
        ...
        def _is_effectively_empty(cell: Tag | None) -> bool:
            ...
            if _cell_text(cell):
                return False
            if cell.find(["img", "svg", "canvas", "table"], recursive=True) is not None:
                return False
            return True
...
def _looks_like_continuation_cell_text(text: str) -> bool:
    ...
    if re.match(r"^(?:[•▪■◆▶◇·ㆍ\-*]|\(?\d+[\.\)]|[가-하][\.\)])\s*", normalized):
        return True
    if re.match(r"^[가-힣A-Za-z0-9]", normalized):
        return True
    return False
    ...
    for line in normalized_lines:
        ...
        if re.match(
            ...
            if re.match(r"^[•▪■◆▶◦○□◇·ㆍ−–—\-\*]+\s+", line):
                ...
                if len(bullet_body) > 14 and re.search(r"(해야|하여야|제공|지원|수행|구성|관리|검증|확인|포함|필요|제시|운영)", bullet_body):
                    content_like += 1
                    continue
            label_like += 1
            continue
...
def _table_fragment_continuity_score(lines: list[str]) -> float:
    ...
    for line in normalized_lines:
        ...
        if re.match(r"^[•▪■◆▶◦○□◇·ㆍ−–—\-\*]+\s+", line) and (
            ...
            or (
                len(compact) <= 24
                and not re.search(r"(해야|하여야|제공|지원|수행|구성|관리|검증|확인|포함|필요|제시|운영)", compact)
            )
        ):
            label_like += 1
            continue
...
def _looks_like_right_only_table_fragment(lines: list[str]) -> bool:
    ...
    for line in normalized_lines:
        ...
        if (
            ...
            or re.search(r"(해야|하여야|제공|지원|수행|구성|관리|검증|확인|포함|필요|제시|운영)", compact)
        ):
            sentence_like += 1
...
def _line_looks_like_table_label(line: str) -> bool:
    ...
    if not compact:
        return False
    if re.match(r"^(?:①|②|③|④|⑤|⑥|⑦|⑧|⑨|⑩|\(?\d+(?:\.\d+)*[\)\.\-]|[가나다라마바사아자차카타파하][\.\)]|[IVXLCDM]+[\)\.])\s+", compact, flags=re.IGNORECASE):
        return True
...
def _table_fragment_lines_to_html(lines: list[str]) -> str:
    ...
    if not rows:
        return ""
    table_rows = "".join(f"<tr><td>{escape(label)}</td><td>{content}</td></tr>" for label, content in rows if label)
...
def _append_cell_children(target: Tag, source: Tag) -> None:
    if _clean_text(target.get_text(" ", strip=True)) and _clean_text(source.get_text(" ", strip=True)):
...
def _analyze_split_row_merge(previous_cells: list[Tag], current_cells: list[Tag]) -> tuple[list[int], bool]:
    ...
    for idx in curr_non_empty:
        ...
        if short_suffix_continuation:
            ...
            continue
        if _looks_like_continuation_cell_text(curr_text) and not _cell_text_ends_like_complete_statement(prev_text):
            mergeable_cols.append(idx)
            continue
...
def _merge_clear_split_row_pair(previous_row: Tag, current_row: Tag, *, require_short_suffix: bool = False) -> bool:
...
def _merge_clear_split_row_continuation(previous_table: Tag, current_table: Tag) -> bool:
    prev_rows = [row for row in previous_table.find_all("tr") if row.find_parent("table") is previous_table]
    curr_rows = [row for row in current_table.find_all("tr") if row.find_parent("table") is current_table]
...
def _normalize_split_rows_within_table(table: Tag) -> bool:
    ...
    while True:
        ...
        for row_idx in range(1, len(rows)):
            if _merge_clear_split_row_pair(rows[row_idx - 1], rows[row_idx], require_short_suffix=True):
                changed = True
                pass_changed = True
                break
        if not pass_changed:
    ...
    for block in content_list:
        ...
        if block_type == "table" and merged and merged[-1].get("type") == "table":
            ...
            if not prev.get("text"):
                prev["text"] = _clean_text(BeautifulSoup(prev["html"], "html.parser").get_text(" ", strip=True))
            continue

        if promoted_html and merged and merged[-1].get("type") == "table" and _page_indices_are_contiguous(merged[-1], block):
            prev = merged[-1]
            prev_html = _extract_table_html(prev)
...
def merge_consecutive_tables_in_html(html: str) -> str:
    ...
    def _merge_tables_in_parent(parent: Tag) -> bool:
        ...
        while idx < len(children):
            ...
            if not _is_tableish_node(node) and node.find("table") is not None:
                if _merge_tables_in_parent(node):
                    changed = True
                    children = list(parent.contents)
                    continue
            ...
            while j < len(children):
                ...
                if not _should_merge_tables(prev_table, curr_table):
                    break
                merged_html = _merge_table_fragments_preserving_outer_html(str(prev_table), str(curr_table))
    ...
    def _merge_tables_in_parent(parent: Tag) -> bool:
        ...
        while idx < len(children):
            ...
            while j < len(children):
                ...
                if prev_table is None or curr_table is None:
                    break
                merged_html = _append_table_fragments_preserving_outer_html(str(prev_table), str(curr_table))
                merged_table = BeautifulSoup(merged_html, "html.parser").find("table")
                if merged_table is None:
                    break
                if not _replace_table_safely(prev_table, merged_table):
...
def restore_table_fragments_in_html_raw(html: str) -> str:
    ...
    def _is_heading_like_fragment_start(text: str) -> bool:
        ...
        if not normalized:
            return False
        return bool(
            re.match(r"^(?:\d+(?:\.\d+)+\.?\s+|[IVXLCDM]+\.\s+|제\s*\d+\s*(?:장|절|항)\s+)", normalized, flags=re.IGNORECASE)
        )
    ...
    def _should_restore_fragment_as_right_only(prev_table: Tag) -> bool:
        ...
        second_text = _clean_text(last_row_cells[1].get_text(" ", strip=True)) if len(last_row_cells) > 1 else ""
        return (not first_text and bool(second_text)) or _last_row_right_cell_is_incomplete(prev_table)

    def _trailing_fragment_lines_to_table(lines: list[str], *, right_only: bool = False) -> Tag | None:
        normalized_lines = [_clean_text(line) for line in lines if _clean_text(line)]
        if len(normalized_lines) < 2:
            return None
        if right_only:
        ...
        table_html = f"<table><tbody><tr><td>{escape(label)}</td><td>{content_html}</td></tr></tbody></table>"
        return BeautifulSoup(table_html, "html.parser").find("table")
    ...
    def _looks_like_label_only_paragraph(text: str) -> bool:
        ...
        if len(normalized.split()) > 4:
            return False
        if any(token in normalized for token in ("합니다", "하여야", "필요", "수행", "제공", "반영", "관리", "검토")):
            return False
        return True
    ...
    def _header_only_following_nodes_to_table(group_nodes: list[Tag]) -> Tag | None:
        ...
        for idx, node in enumerate(group_nodes[:-1]):
            ...
            if len(current_lines) == 1 and next_lines and _looks_like_label_only_paragraph(current_lines[0]):
                has_label_pair = True
                break
        ...
        for label, content_nodes in rows:
            ...
            if not _clean_text(BeautifulSoup(content_html, "html.parser").get_text(" ", strip=True)):
                continue
            row_html_parts.append(f"<tr><td>{escape(label)}</td><td>{content_html}</td></tr>")

        if not row_html_parts:
            return None
        return BeautifulSoup(f"<table><tbody>{''.join(row_html_parts)}</tbody></table>", "html.parser").find("table")
        ...
        while idx < len(children):
            ...
            if _is_header_only_two_col_table(prev_table):
                ...
                if promoted_header_tail is not None:
                    merged_html = _append_table_fragments_preserving_outer_html(str(prev_table), str(promoted_header_tail))
                    merged_table = BeautifulSoup(merged_html, "html.parser").find("table")
            ...
            merged_html = _append_table_fragments_preserving_outer_html(str(prev_table), str(promoted_table))
            merged_table = BeautifulSoup(merged_html, "html.parser").find("table")
            if merged_table is None:
                idx += 1
                continue
...
def build_html_document(content_list: list[dict], markdown_path: Path | None) -> str:
    ...
    if content_list:
        ...
        for index, block in enumerate(render_blocks):
            ...
            if isinstance(page_idx, int) and isinstance(page_end_idx, int) and page_end_idx != page_idx:
            ...
            if block_type in {"text", "list"}:
                ...
                if promoted_html:
                    ...
                    if table_root is not None:
                        changed = True
                        while changed:
                            changed = _merge_empty_cells_vertically(table_root) or _copy_empty_cells_from_above(table_root)
            ...
            if block_type in {"text", "title"} and text:
                if text_level > 0:
                    ...
                    parts.append(
                        f"<p data-page='{page_idx}'>{escape(text)}</p>"
                    )
