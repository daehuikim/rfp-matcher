...
from rfpmatch.toc_parser import _extract_body_headings, _extract_toc_from_area, split_toc_body_html_by_toc_items


def toc_items_to_rows(items: list[TocItem], page_source_map: dict[str, str] | None = None) -> list[dict]:
    return [
        {
            "level": item.level,
            "title": item.title,
...
def rows_to_toc_items(rows: list[dict]) -> list[TocItem]:
    ...
    for row in rows:
        ...
        estimate_raw = row.get("page_estimate", row.get("pageEstimate", row.get("page_estimated", None)))
        try:
        ...
        updated_items.append(TocItem(level=min(max(level, 1), 3), title=title, page_idx=page_idx, page_estimate=page_estimate, anchor=anchor))
...
def fill_missing_pages_from_neighbors(items: list[TocItem]) -> list[TocItem]:
    ...
    for idx, item in enumerate(items):
        ...
        if page_idx is not None:
            last_page = page_idx
        filled.append(TocItem(level=item.level, title=item.title, page_idx=page_idx, page_estimate=item.page_estimate, anchor=item.anchor))
    return filled


def reconcile_pages_from_candidates(items: list[TocItem], candidates: list[TocItem]) -> list[TocItem]:
    ...
    for item in items:
        key = normalize_title_for_match(item.title)
        page_idx = page_map.get(key, item.page_idx if item.page_idx is not None else item.page_estimate)
        page_estimate = item.page_estimate if item.page_estimate is not None else estimate_map.get(key)
        if page_idx is None and page_estimate is not None:
            page_idx = page_estimate
        fixed.append(
            TocItem(
...
def append_toc_item(items: list[TocItem], *, title: str = "새 항목") -> list[TocItem]:
    return [*items, TocItem(level=1, title=title or "새 항목", page_idx=None, page_estimate=None, anchor=slugify(title or "새 항목"))]
...
def extract_leading_numbering(title: str) -> str:
    ...
    match = re.match(r"^(제\s*\d+\s*(?:장|절|항)|\d+(?:\.\d+)*)(?:[\.\-\)]\s*|\s+)", compact, flags=re.IGNORECASE)
    if not match:
        return ""
...
def reconcile_pages_from_candidates(items: list[TocItem], candidates: list[TocItem]) -> list[TocItem]:
    ...
    for item in candidates:
        ...
        if not key:
            continue
        if item.page_idx is not None and key not in page_map:
            page_map[key] = item.page_idx
        if item.page_estimate is not None and key not in estimate_map:
            estimate_map[key] = item.page_estimate
    fixed: list[TocItem] = []
    for item in items:
        key = normalize_title_for_match(item.title)
        page_idx = page_map.get(key, item.page_idx)
        page_estimate = item.page_estimate
        if page_estimate is None:
            page_estimate = estimate_map.get(key)
        fixed.append(TocItem(level=item.level, title=item.title, page_idx=page_idx, page_estimate=page_estimate, anchor=item.anchor))
    return fixed
...
def filter_items_with_page(items: list[TocItem]) -> tuple[list[TocItem], int]:
    return fill_missing_pages_from_neighbors(list(items)), 0
...
def refresh_step2_split_views_from_final_toc() -> bool:
    html = ""
    source_key = ""
    if st.session_state.get("html_raw_merged_empty_up_postprocessed"):
        ...
        source_key = "html_raw_merged_empty_up_postprocessed"
    elif st.session_state.get("html_merged_from_raw_postprocessed"):
        html = st.session_state.get("html_merged_from_raw_postprocessed") or ""
        source_key = "html_merged_from_raw_postprocessed"
    elif st.session_state.get("html_merged_from_raw"):
        html = st.session_state.get("html_merged_from_raw") or ""
        source_key = "html_merged_from_raw"
    ...
    if not html or toc_items is None:
        return False
    if not toc_items:
        st.session_state["step2_toc_html"] = ""
        st.session_state["step2_body_html"] = html
        st.session_state["step2_split_source"] = source_key
        st.session_state["toc_area_items"] = []
        st.session_state["toc_body_items"] = []
        return True
    toc_html, body_html = split_toc_body_html_by_toc_items(html, toc_items)
    st.session_state["step2_toc_html"] = toc_html
    st.session_state["step2_body_html"] = body_html
    st.session_state["step2_split_source"] = source_key
    toc_area_items = []
    toc_body_items = []
    if toc_html.strip():
        toc_area_items = _extract_toc_from_area(BeautifulSoup(toc_html, "html.parser"))
    if body_html.strip():
        toc_body_items = _extract_body_headings(BeautifulSoup(body_html, "html.parser"))
    st.session_state["toc_area_items"] = toc_area_items
    st.session_state["toc_body_items"] = toc_body_items
    if st.session_state.get("saved_toc_items") is None:
        combined = drop_toc_heading_items(dedup_toc_items_by_title(toc_area_items + toc_body_items))
        st.session_state["toc_items"] = combined
    return True
